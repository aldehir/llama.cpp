...
// Check if two DFAs are structurally identical
static bool dfas_equal(const byte_dfa & a, const byte_dfa & b) {
    if (a.start_state != b.start_state) return false;
...
// Two segments are considered equal for left-factoring purposes.
// For DFA_MATCH, compares actual DFA content since identical terminals
// may have been compiled to different DFA IDs.
static bool segments_equal(const compiled_segment & a, const compiled_segment & b,
                           const std::vector<byte_dfa> & dfas) {
    if (a.type != b.type) return false;
    if (a.type == compiled_segment::RULE_CALL) return a.id == b.id;
    // DFA_MATCH: compare by ID first (fast path), then by content
    if (a.id == b.id) return true;
    return dfas_equal(dfas[a.id], dfas[b.id]);
}

// Compute the set of first bytes accepted by a DFA from its start state
static std::bitset<256> dfa_first_bytes(const byte_dfa & dfa) {
    std::bitset<256> result;
    for (int b = 0; b < 256; b++) {
        if (dfa.transitions[dfa.start_state][b] != 0) {
            result.set(b);
        }
    }
    return result;
...
// Extract common DFA prefix from multiple DFAs via lockstep simulation.
// Returns true if a non-empty common prefix was found.
// On success, prefix_dfa contains the shared prefix and suffix_dfas
// contain per-input remainders (original DFA with start state changed).
static bool extract_common_dfa_prefix(
        const std::vector<byte_dfa> & all_dfas,
        const std::vector<uint32_t> & dfa_ids,
        byte_dfa & prefix_dfa,
        std::vector<byte_dfa> & suffix_dfas) {
    size_t n = dfa_ids.size();
    if (n < 2) return false;

    // Initialize prefix DFA with dead state
    prefix_dfa = byte_dfa();
    prefix_dfa.transitions.emplace_back();
    prefix_dfa.transitions[0].fill(0);
    prefix_dfa.accept.push_back(false);

    // Create start state
    uint16_t cur_prefix = (uint16_t) prefix_dfa.transitions.size();
    prefix_dfa.transitions.emplace_back();
    prefix_dfa.transitions[cur_prefix].fill(0);
    prefix_dfa.accept.push_back(false);
    prefix_dfa.start_state = cur_prefix;

    // Current states for each input DFA
    std::vector<uint16_t> current(n);
    for (size_t i = 0; i < n; i++) {
        current[i] = all_dfas[dfa_ids[i]].start_state;
    }

    bool found_any = false;

    while (true) {
        // Stop if any DFA is at an accept state (prefix ends before this byte)
        bool any_accept = false;
        for (size_t i = 0; i < n; i++) {
            if (all_dfas[dfa_ids[i]].accept[current[i]]) {
                any_accept = true;
                break;
            }
        }
        ...
        // Check if all DFAs agree on which bytes are valid (non-dead)
        bool agree = true;
        for (int b = 0; b < 256 && agree; b++) {
            bool first_dead = (all_dfas[dfa_ids[0]].transitions[current[0]][b] == 0);
            for (size_t i = 1; i < n; i++) {
                bool this_dead = (all_dfas[dfa_ids[i]].transitions[current[i]][b] == 0);
                if (this_dead != first_dead) {
                    agree = false;
                    break;
                }
            }
        ...
        // Group valid bytes by the state tuple they produce
        // For the prefix to be well-defined, require all valid bytes lead to
        // the same next state tuple (handles string literals and simple char ranges)
        std::vector<uint16_t> next_tuple(n, 0);
        bool has_valid = false;
        bool single_tuple = true;

        for (int b = 0; b < 256; b++) {
            if (all_dfas[dfa_ids[0]].transitions[current[0]][b] == 0) continue;

            std::vector<uint16_t> this_tuple(n);
            for (size_t i = 0; i < n; i++) {
                this_tuple[i] = all_dfas[dfa_ids[i]].transitions[current[i]][b];
            }

            if (!has_valid) {
                next_tuple = this_tuple;
                has_valid = true;
            } else if (this_tuple != next_tuple) {
                single_tuple = false;
                break;
            }
        }

        if (!has_valid || !single_tuple) break;

        // Extend prefix DFA: create next state and add transitions
        uint16_t next_prefix = (uint16_t) prefix_dfa.transitions.size();
        prefix_dfa.transitions.emplace_back();
        prefix_dfa.transitions[next_prefix].fill(0);
        prefix_dfa.accept.push_back(false);

        for (int b = 0; b < 256; b++) {
            if (all_dfas[dfa_ids[0]].transitions[current[0]][b] != 0) {
                prefix_dfa.transitions[cur_prefix][b] = next_prefix;
            }
        }
    ...
    // Mark last prefix state as accept
    prefix_dfa.accept[cur_prefix] = true;

    // Build suffix DFAs: copies of originals with start state at divergence point
    suffix_dfas.resize(n);
    for (size_t i = 0; i < n; i++) {
        suffix_dfas[i] = all_dfas[dfa_ids[i]];
        suffix_dfas[i].start_state = current[i];
    }

    return true;
}

// Remove duplicate suffix alternates
static void dedup_suffix_alternates(
        std::vector<std::vector<compiled_segment>> & alternates,
        const std::vector<byte_dfa> & dfas) {
    std::vector<std::vector<compiled_segment>> unique;
        ...
        for (const auto & u : unique) {
            if (s.size() == u.size()) {
                ...
                for (size_t k = 0; k < s.size(); k++) {
                    if (!segments_equal(s[k], u[k], dfas)) {
                        equal = false;
                        break;
                    }
                }
    ...
    alternates = std::move(unique);
}

// Factor a group of alternates that share a common segment-level prefix.
// Appends the factored alternate to new_alternates.
// May add new rules and DFAs to the grammar.
static void factor_segment_group(
        const std::vector<uint32_t> & group,
        const std::vector<std::vector<compiled_segment>> & alternates,
        std::vector<std::vector<compiled_segment>> & new_alternates,
        std::vector<compiled_rule> & rules,
        std::vector<byte_dfa> & dfas) {

    // Find longest common prefix across all alternates in this group
    size_t prefix_len = alternates[group[0]].size();
    for (size_t i = 1; i < group.size(); i++) {
        const auto & a = alternates[group[0]];
        const auto & b = alternates[group[i]];
        size_t common = 0;
        size_t max_len = std::min(a.size(), b.size());
        while (common < max_len && segments_equal(a[common], b[common], dfas)) {
            common++;
        }
        prefix_len = std::min(prefix_len, common);
    }

    assert(prefix_len >= 1);

    // Build suffix alternates
    compiled_rule suffix_rule;
    for (uint32_t idx : group) {
        const auto & alt = alternates[idx];
        std::vector<compiled_segment> suffix(alt.begin() + prefix_len, alt.end());
        suffix_rule.alternates.push_back(std::move(suffix));
    }

    dedup_suffix_alternates(suffix_rule.alternates, dfas);

    uint32_t suffix_rule_id = (uint32_t) rules.size();
    rules.push_back(std::move(suffix_rule));

    // Build new alternate: prefix + RULE_CALL(suffix_rule)
    std::vector<compiled_segment> new_alt(
        alternates[group[0]].begin(),
        alternates[group[0]].begin() + prefix_len);

    // Only add suffix call if there's non-trivial suffix content
    bool all_suffixes_empty = true;
    for (const auto & s : rules[suffix_rule_id].alternates) {
        if (!s.empty()) {
            all_suffixes_empty = false;
            break;
        }
    }

    if (!all_suffixes_empty) {
        compiled_segment call_seg;
        call_seg.type = compiled_segment::RULE_CALL;
        call_seg.id   = suffix_rule_id;
        new_alt.push_back(call_seg);
    }

    new_alternates.push_back(std::move(new_alt));
}

// Phase 1: DFA prefix merging.
// For alternates starting with DFA_MATCH that share the same first-byte set but have
// different DFAs, extract the common byte-level prefix and split them.
// Modifies the rule in-place, may add new DFAs and rules.
// Returns true if any merging was done.
static bool dfa_prefix_merge(
        uint32_t rule_id,
        std::vector<byte_dfa> & dfas,
        std::vector<compiled_rule> & rules) {
    // Snapshot alternates to avoid invalidation when rules/dfas vectors grow
    auto orig_alts = rules[rule_id].alternates;
    if (orig_alts.size() <= 1) return false;

    // Group alternates starting with DFA_MATCH by first-byte set
    struct dfa_group {
        std::bitset<256>        first_bytes;
        std::vector<uint32_t>   alt_indices;
    };
    std::vector<dfa_group> dfa_groups;
    std::vector<uint32_t> non_dfa_alts;  // alternates not starting with DFA_MATCH

    for (uint32_t i = 0; i < (uint32_t) orig_alts.size(); i++) {
        const auto & alt = orig_alts[i];
        if (alt.empty() || alt[0].type != compiled_segment::DFA_MATCH) {
            non_dfa_alts.push_back(i);
            continue;
        }

        auto fb = dfa_first_bytes(dfas[alt[0].id]);
        bool found = false;
        for (auto & dg : dfa_groups) {
            if (dg.first_bytes == fb) {
                dg.alt_indices.push_back(i);
                found = true;
                break;
            }
        }
        if (!found) {
            dfa_groups.push_back({fb, {i}});
        }
    }

    // Check if any DFA group has >1 member with non-identical DFAs
    bool any_mergeable = false;
    for (const auto & dg : dfa_groups) {
        if (dg.alt_indices.size() < 2) continue;
        // Check if DFAs are already identical (would be handled by segment-level factoring)
        bool all_identical = true;
        uint32_t first_dfa = orig_alts[dg.alt_indices[0]][0].id;
        for (size_t j = 1; j < dg.alt_indices.size(); j++) {
            if (!dfas_equal(dfas[first_dfa], dfas[orig_alts[dg.alt_indices[j]][0].id])) {
                all_identical = false;
                break;
            }
        }
    ...
    if (!any_mergeable) return false;

    // Perform DFA prefix merging
    std::vector<std::vector<compiled_segment>> new_alternates;

    for (const auto & dg : dfa_groups) {
        if (dg.alt_indices.size() < 2) {
            // Singleton — keep as-is
            for (uint32_t idx : dg.alt_indices) {
                new_alternates.push_back(orig_alts[idx]);
            }
            continue;
        }

        // Collect DFA IDs
        std::vector<uint32_t> dfa_ids;
        for (uint32_t idx : dg.alt_indices) {
            dfa_ids.push_back(orig_alts[idx][0].id);
        }

        byte_dfa prefix_dfa;
        std::vector<byte_dfa> suffix_dfa_list;
        if (!extract_common_dfa_prefix(dfas, dfa_ids, prefix_dfa, suffix_dfa_list)) {
        ...
        // Add prefix DFA
        uint32_t prefix_dfa_id = (uint32_t) dfas.size();
        dfas.push_back(std::move(prefix_dfa));

        // Build suffix rule
        compiled_rule suffix_rule;
        for (size_t k = 0; k < dg.alt_indices.size(); k++) {
            const auto & orig_alt = orig_alts[dg.alt_indices[k]];
            std::vector<compiled_segment> suffix_alt;

            // Add suffix DFA if prefix didn't consume the entire original DFA
            const auto & sdfa = suffix_dfa_list[k];
            if (!sdfa.accept[sdfa.start_state]) {
                uint32_t suffix_dfa_id = (uint32_t) dfas.size();
                dfas.push_back(sdfa);
                suffix_alt.push_back({compiled_segment::DFA_MATCH, suffix_dfa_id});
            }

            // Append remaining segments from original alternate
            for (size_t s = 1; s < orig_alt.size(); s++) {
                suffix_alt.push_back(orig_alt[s]);
            }

            suffix_rule.alternates.push_back(std::move(suffix_alt));
        }

        dedup_suffix_alternates(suffix_rule.alternates, dfas);

        // Build factored alternate
        std::vector<compiled_segment> new_alt;
        new_alt.push_back({compiled_segment::DFA_MATCH, prefix_dfa_id});

        bool all_suffixes_empty = true;
        for (const auto & s : suffix_rule.alternates) {
            if (!s.empty()) { all_suffixes_empty = false; break; }
        }

        if (!all_suffixes_empty) {
            uint32_t suffix_rule_id = (uint32_t) rules.size();
            rules.push_back(std::move(suffix_rule));
            new_alt.push_back({compiled_segment::RULE_CALL, suffix_rule_id});
        }

        new_alternates.push_back(std::move(new_alt));
    }

    // Add back non-DFA alternates
    for (uint32_t idx : non_dfa_alts) {
        new_alternates.push_back(orig_alts[idx]);
    }

    rules[rule_id].alternates = std::move(new_alternates);
    return true;
}

bool compiled_grammar::left_factor_rule(uint32_t rule_id) {
    bool changed = false;

    // Phase 1: DFA prefix merging for terminal-heavy alternates
    changed |= dfa_prefix_merge(rule_id, dfas, rules);

    if (rules[rule_id].alternates.size() <= 1) {
        return changed;
    }

    // Phase 2: Segment-level factoring (handles RULE_CALL and identical DFA_MATCH prefixes)
    // Snapshot alternates since factor_segment_group may grow the rules vector
    auto alts = rules[rule_id].alternates;

    std::vector<std::vector<uint32_t>> groups;

    for (uint32_t i = 0; i < (uint32_t) alts.size(); i++) {
        ...
        for (size_t g = 0; g < groups.size(); g++) {
            const auto & rep = alts[groups[g][0]];
            if (!rep.empty() && segments_equal(alt[0], rep[0], dfas)) {
    ...
    for (const auto & group : groups) {
        if (group.size() == 1) {
            new_alternates.push_back(alts[group[0]]);
            continue;
        }

        factor_segment_group(group, alts, new_alternates, rules, dfas);
    }
...
void compiled_grammar::left_factor() {
    // Process all rules, including newly created suffix rules.
    // Use index-based loop since rules vector may grow.
    for (size_t i = 0; i < rules.size(); i++) {
        left_factor_rule((uint32_t) i);
    }
}
