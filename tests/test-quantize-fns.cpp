...
static std::string error_msg(const char * what, float error, float max_error) {
    char buf[128];
    snprintf(buf, sizeof(buf), "%s %f < %f", what, error, max_error);
    return buf;
}

static void test_vec_dot_f32(testing & t, bool verbose) {
    const auto * f32 = ggml_get_type_traits_cpu(GGML_TYPE_F32);
    for (int n : {1, 2, 3, 5, 7, 8, 15, 16, 17, 31, 33, 63, 67, 127, 129, 193, 255, 1023}) {
        t.test("n=" + std::to_string(n), [&](testing & t) {
            ...
            t.assert_true(error_msg("f32 vec_dot error", error, MAX_QUANTIZATION_REFERENCE_ERROR), !failed);
        });
    }
}

static void test_vec_dot_q_type(testing & t, ggml_type type, bool verbose, size_t test_size,
                                const float * test_data, const float * test_data2, const float * test_data3, const float * test_data4) {
    ...
    if (!(qfns_cpu->from_float && qfns->to_float)) {
        t.skip("no from_float or to_float");
        return;
    }
    ...
    t.assert_true(error_msg("absolute quantization error", total_error, max_quantization_error), !failed);
    ...
    t.assert_true(error_msg("reference implementation error", reference_error, MAX_QUANTIZATION_REFERENCE_ERROR), !failed);
    ...
    t.assert_true(error_msg("dot product error", vec_dot_error, max_allowed_error), !failed);

    // Test nrc=2 path for types that support it
    if (qfns_cpu->nrows == 2) {
        const float vec_dot_error_nrc2 = dot_product_error(qfns_cpu, type, test_size, test_data, test_data2, test_data3, test_data4, 2);
        ...
        t.assert_true(error_msg("dot product error (nrc=2)", vec_dot_error_nrc2, max_allowed_error), !failed);
    }
}

static void test_vec_dot_q(testing & t, bool verbose) {
    ...
    for (int i = 0; i < GGML_TYPE_COUNT; i++) {
        ...
        t.test(ggml_type_name(type), [&](testing & t) {
            test_vec_dot_q_type(t, type, verbose, test_size, test_data.data(), test_data2.data(), test_data3.data(), test_data4.data());
        });
    }
}
...
int main(int argc, char * argv[]) {
    ...
    t.test("vec_dot_f32", [&](testing & t) {
        test_vec_dot_f32(t, verbose);
    });

    t.test("vec_dot_q", [&](testing & t) {
        test_vec_dot_q(t, verbose);
    });
