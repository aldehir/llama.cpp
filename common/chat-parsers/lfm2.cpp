...
static common_chat_params init_lfm2(const common_chat_template & tmpl, common_chat_render_inputs & inputs) {
    return common_chat_params_init_lfm2(tmpl, inputs, /* tool_list_tokens = */ true);
}

static common_chat_params init_lfm2_5(const common_chat_template & tmpl, common_chat_render_inputs & inputs) {
    return common_chat_params_init_lfm2(tmpl, inputs, /* tool_list_tokens = */ false);
}

static bool detect(const std::string & src) {
    return is_lfm2_template(src);
}

// LFM2.5 format detection: template uses plain "List of tools: [...]" with no special tokens
static bool detect_lfm2_5(const std::string & src) {
    return src.find("List of tools: [") != std::string::npos &&
           src.find("<|tool_list_start|>") == std::string::npos;
}

const common_chat_parser_def common_chat_parser_lfm2   = { "lfm2", detect, init_lfm2 };
const common_chat_parser_def common_chat_parser_lfm2_5 = { "lfm2.5", detect_lfm2_5, init_lfm2_5 };
