...
static common_chat_params common_chat_params_init_gemma4(const common_chat_template &      tmpl,
    ...
    if (src.find("{#- OpenAI Chat Completions:") == std::string::npos) {
        ...
        convert_tool_responses_gemma4(inputs.messages);
    }
...
static bool detect(const std::string & src) {
    return src.find("'<|tool_call>call:'") != std::string::npos;
}

const common_chat_parser_def common_chat_parser_gemma4 = { "gemma4", detect, common_chat_params_init_gemma4 };
