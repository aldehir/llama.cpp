...
private:
    ...
    statement_ptr parse_any() {
        switch (peek().t) {
            case token::comment:
                return std::make_shared<comment_statement>(tokens[current++].value);
            case token::text:
                return std::make_shared<string_literal>(tokens[current++].value);
    ...
    statement_ptr parse_jinja_statement() {
        ...
        if (name == "set") {
            ...
            expect(token::close_statement, "Expected %}");
            result = std::make_shared<break_statement>();

        } else if (name == "continue") {
            expect(token::close_statement, "Expected %}");
            result = std::make_shared<continue_statement>();
            ...
            result = std::make_shared<call_statement>(std::move(call_expr), std::move(caller_args), std::move(body));
            ...
            expect(token::close_statement, "Expected %}");
            result = std::make_shared<filter_statement>(std::move(filter_node), std::move(body));
    ...
    statement_ptr parse_set_statement() {
        ...
        expect(token::close_statement, "Expected %}");
        return std::make_shared<set_statement>(std::move(left), std::move(value), std::move(body));
    }
    ...
    statement_ptr parse_expression_sequence(bool primary = false) {
        ...
        return is_tuple ? std::make_shared<tuple_literal>(std::move(exprs)) : std::move(exprs[0]);
    }
    ...
    statement_ptr parse_for_statement() {
        ...
        return std::make_shared<for_statement>(
    ...
    statement_ptr parse_if_expression() {
        ...
        if (is_identifier("if")) {
            ...
            if (is_identifier("else")) {
                ...
                return std::make_shared<ternary_expression>(std::move(test), std::move(a), std::move(false_expr));
            } else {
                // Select expression on iterable
                return std::make_shared<select_expression>(std::move(a), std::move(test));
            }
        }
        return a;
    }
    ...
    statement_ptr parse_logical_or_expression() {
        ...
        while (is_identifier("or")) {
            ...
            left = std::make_shared<binary_expression>(op, std::move(left), parse_logical_and_expression());
        }
        return left;
    }
    ...
    statement_ptr parse_logical_and_expression() {
        ...
        while (is_identifier("and")) {
            ...
            left = std::make_shared<binary_expression>(op, std::move(left), parse_logical_negation_expression());
        }
        return left;
    }
    ...
    statement_ptr parse_logical_negation_expression() {
        ...
        if (is_identifier("not")) {
            auto op = tokens[current];
            ++current; // consume 'not'
            return std::make_shared<unary_expression>(op, parse_logical_negation_expression());
        }
    ...
    statement_ptr parse_comparison_expression() {
        ...
        while (true) {
            ...
            left = std::make_shared<binary_expression>(op, std::move(left), parse_additive_expression());
        }
        return left;
    }
    ...
    statement_ptr parse_additive_expression() {
        ...
        while (is(token::additive_binary_operator)) {
            auto op = tokens[current++];
            left = std::make_shared<binary_expression>(op, std::move(left), parse_multiplicative_expression());
        }
        return left;
    }
    ...
    statement_ptr parse_multiplicative_expression() {
        ...
        while (is(token::multiplicative_binary_operator)) {
            auto op = tokens[current++];
            left = std::make_shared<binary_expression>(op, std::move(left), parse_test_expression());
        }
        return left;
    }
    ...
    statement_ptr parse_test_expression() {
        ...
        while (is_identifier("is")) {
            ...
            operand = std::make_shared<test_expression>(std::move(operand), negate, std::move(test_id));
        }
        return operand;
    }
    ...
    statement_ptr parse_filter_expression() {
        ...
        while (is(token::pipe)) {
            ...
            operand = std::make_shared<filter_expression>(std::move(operand), std::move(filter));
        }
        return operand;
    }
    ...
    statement_ptr parse_call_expression(statement_ptr callee) {
        auto expr = std::make_shared<call_expression>(std::move(callee), parse_args());
    ...
    statements parse_args() {
        ...
        while (!is(token::close_paren)) {
            ...
            if (peek().t == token::multiplicative_binary_operator && peek().value == "*") {
                ++current; // consume *
                arg = std::make_shared<spread_expression>(parse_expression());
            } else {
                ...
                if (is(token::equals)) {
                    ...
                    arg = std::make_shared<keyword_argument_expression>(std::move(arg), parse_expression());
                }
            }
    ...
    statement_ptr parse_member_expression(statement_ptr object) {
        while (is(token::dot) || is(token::open_square_bracket)) {
            ...
            object = std::make_shared<member_expression>(std::move(object), std::move(prop), computed);
        }
        return object;
    }
    ...
    statement_ptr parse_primary_expression() {
        ...
        switch (t.t) {
            case token::numeric_literal:
                if (t.value.find('.') != std::string::npos) return std::make_shared<float_literal>(std::stod(t.value));
                return std::make_shared<integer_literal>(std::stoll(t.value));
            ...
            case token::open_square_bracket: {
                ...
                current++;
                return std::make_shared<array_literal>(std::move(vals));
            }
