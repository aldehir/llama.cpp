            ...
            foreach_function(inputs.tools, [&](const json & tool) {
                ...
                foreach_parameter(function, [&](const common_schema_property & param, const common_schema_document_ptr & doc) {
                    ...
                    if (!types.has(common_schema::TYPE_STRING)) {
                        ...
                        if (types.has(common_schema::TYPE_OBJECT)) {
                            json_value |= p.json_object();
                        }
                        if (types.has(common_schema::TYPE_ARRAY)) {
                            json_value |= p.json_array();
                        }
                        if (types.has(common_schema::TYPE_NUMBER) || types.has(common_schema::TYPE_INTEGER)) {
                            json_value |= p.json_number();
                        }
                        if (types.has(common_schema::TYPE_BOOLEAN)) {
                            json_value |= p.json_bool();
                        }
                        if (types.has(common_schema::TYPE_NULL)) {
