...
gguf_split_info get_gguf_split_info(const std::string & path) {
    ...
    static const auto split_parser = build_peg_parser([](auto & p) {
        ...
        return p.tag("prefix", p.zero_or_more(p.negate(split) + p.any())) + split;
    });

    // [-.]TAG at end of string
    static const auto tag_parser = build_peg_parser([](auto & p) {
        auto suffix = p.chars("[.-]", 1, 1) + p.chars("[a-zA-Z0-9_]", 1) + p.end();
        return p.zero_or_more(p.negate(suffix) + p.any())
